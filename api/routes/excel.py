# api/routes/excel.py - FIXED IMPORTS
"""
Excel RAG Routes - Compatible with original IP-based token_db
"""
from fastapi import APIRouter, HTTPException, Request

from src.excel.rag_engine import excel_rag_engine  # FIXED: src not data
...
from src.models.schemas import ExcelQueryRequest, ExcelQueryResponse
...
@router.post("/ask", response_model=ExcelQueryResponse)
async def ask_excel(request: Request, query: ExcelQueryRequest):
    ...
    return ExcelQueryResponse(
        ...
        remembered_entities=result["remembered_entities"],
...
async def excel_token_status(request: Request, chat_id: str):
    """Get token status for excel module (IP-based)"""
    client_ip = get_client_ip(request)
    user_data = get_or_create_user(client_ip)
    user_id = user_data['user']['id']
    token_info = get_user_tokens(user_id)
